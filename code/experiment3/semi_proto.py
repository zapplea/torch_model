...
class Net(tr.nn.Module):
    ...
    def forward_unlabeled(self,U):
        """
        mapping unlabeled instances to another embedding space
        :param U: shape=(batch size, feature height dim, feature width dim)
    ...
    def partial_weight(self,P,U):
        """
        calcualte the weight for partial assignment
        :param P: shape = (labels dim, connect feature dim)
        :param U: shape = (-1, connect feature dim)
        :return: (num of unlabeled, labels num , connect feature dim)
        """
        # shape=(number of unlabeled, 1, connect feature dim)
        U = tr.unsqueeze(U, dim=1)
        # shape=(number of unlabeled, label dim, connect feature dim)
        U = U.repeat(1,self.nn_config['label_dim'],1)

        # shape = (number of unlabeled, labels num)
        euclidean_distance = tr.mul(tr.add(U, -P), tr.add(U, -P)).sum(2)
        # shape = (number of unlabeled, labels num)
    ...
    def forward_softmax(self,X,C,U):
        """
        ...
        # shape = (labels num, k shot, fully connected layer dim)
        C = self.forward_prot(C)

        # calculate prototypes
        # shape = (labels num, connect feature dim)
        P = C.mean(1)
        ...
        # shape = (num of unlabeled, label dim, connect_feature_dim)
        weighted_U = weight*U
        # shape = ( label dim, num of unlabeled, connect_feature_dim)
        weighted_U = tr.transpose(weighted_U,0,1)
        # shape = ( label dim, num of unlabeled+k shot, connect_feature_dim)
        C_hat = tr.cat([C,weighted_U],dim=1)
        P_hat = C_hat.mean(1)

        # shape = (batch size, labels num)
        euclidean_distance = tr.sqrt(tr.mul(tr.add(X,-P_hat),tr.add(X,-P_hat)).sum(2))
